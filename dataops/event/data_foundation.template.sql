...
CREATE OR REPLACE FUNCTION {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.WEB_SEARCH("QUERY" VARCHAR)
RETURNS VARCHAR
LANGUAGE PYTHON
...
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.email_data_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = '|'
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/email_previews_data.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.email_data_stage 
    auto_compress = false 
    overwrite = true;

-- Truncate the EMAIL_PREVIEWS table to avoid duplicates
TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.EMAIL_PREVIEWS;

-- Load data from the CSV file
COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.EMAIL_PREVIEWS (
    EMAIL_ID,
    RECIPIENT_EMAIL,
    SUBJECT,
    HTML_CONTENT,
...
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.email_data_stage/email_previews_data.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = '|'
    SKIP_HEADER = 1
...
FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.EMAIL_PREVIEWS;
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.email_data_stage;

-- Grant permissions on EMAIL_PREVIEWS for SEND_EMAIL_NOTIFICATION procedure
GRANT SELECT, INSERT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.EMAIL_PREVIEWS TO ROLE {{ env.EVENT_ATTENDEE_ROLE }};
GRANT SELECT, INSERT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.EMAIL_PREVIEWS TO ROLE PUBLIC;
...
def send_email(session: snowpark.Session, email_subject: str, email_content: str, 
    ...
    try:
        ...
        # Save email to table
        insert_query = """
            INSERT INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.EMAIL_PREVIEWS 
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.TRANSCRIBED_EARNINGS_CALLS_WITH_SENTIMENT (
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.TRANSCRIPTS_BY_MINUTE (
	RELATIVE_PATH VARCHAR(16777216),
	SPEAKER VARCHAR(16777216),
	SPEAKER_NAME VARCHAR(16777216),
	MINUTES NUMBER(38,0),
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SENTIMENT_ANALYSIS (
	RELATIVE_PATH VARCHAR(16777216),
	SENTIMENT VARCHAR(8),
	OVERALL NUMBER(18,0),
	COST NUMBER(18,0),
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.INFOGRAPHIC_METRICS_EXTRACTED (
	RELATIVE_PATH VARCHAR(16777216),
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.ANALYST_REPORTS_ALL_DATA (
	RELATIVE_PATH VARCHAR(16777216),
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.FINANCIAL_REPORTS (
	RELATIVE_PATH VARCHAR(16777216),
	EXTRACTED_DATA OBJECT
);
...
CREATE OR REPLACE VIEW {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.VW_INFOGRAPHIC_METRICS(
	COMPANY_TICKER,
	RELATIVE_PATH,
	TEXT,
	COMPANY_NAME,
...
SELECT 
    ...
    BRANDING
FROM {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.INFOGRAPHIC_METRICS_EXTRACTED;

CREATE OR REPLACE VIEW {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.VW_FINANCIAL_SUMMARY(
	COMPANY,
	TICKER,
	PERIOD,
...
CREATE OR REPLACE VIEW {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.VW_INCOME_STATEMENT(
	COMPANY,
	TICKER,
	PERIOD,
	LINE_ITEM,
...
CREATE OR REPLACE VIEW {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.VW_KPI_METRICS(
	RELATIVE_PATH,
	COMPANY,
	TICKER,
	PERIOD,
...
CREATE OR REPLACE VIEW {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SENTIMENT_WITH_TRANSCRIPTS_FOR_SEARCH(
	PRIMARY_TICKER,
	SENTIMENT_REASON,
...
parsed_transcripts AS (
    ...
    FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.unique_transcripts
),
speaker_lookup AS (
    SELECT
        primary_ticker,
...
NATURAL JOIN {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ai_transcripts_analysts_sentiments b;

-- Note: STOCK_PRICE_TIMESERIES references external data share - may need adjustment
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.STOCK_PRICES AS
SELECT 
    TICKER,
    ASSET_CLASS,
    PRIMARY_EXCHANGE_CODE,
...
FROM (
    ...
    FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.STOCK_PRICE_TIMESERIES
)
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.INFOGRAPHICS_FOR_SEARCH (
	COMPANY_TICKER VARCHAR(16777216),
	RELATIVE_PATH VARCHAR(16777216),
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ANALYST_REPORTS (
	RELATIVE_PATH VARCHAR(16777216),
	FILE_URL VARCHAR(16777216),
	RATING VARCHAR(16777216),
	DATE_REPORT VARCHAR(16777216),
...
-- =====================================================
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ai_transcripts_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/ai_transcripts_analysts_sentiments.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ai_transcripts_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.AI_TRANSCRIPTS_ANALYSTS_SENTIMENTS;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.AI_TRANSCRIPTS_ANALYSTS_SENTIMENTS
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ai_transcripts_stage/ai_transcripts_analysts_sentiments.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ai_transcripts_stage;
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.TRANSCRIBED_EARNINGS_CALLS (
	RELATIVE_PATH VARCHAR(16777216),
	EXTRACTED_DATA VARIANT
);

CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.transcribed_calls_raw_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/transcribed_earnings_calls.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.transcribed_calls_raw_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.TRANSCRIBED_EARNINGS_CALLS;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.TRANSCRIBED_EARNINGS_CALLS
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.transcribed_calls_raw_stage/transcribed_earnings_calls.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.transcribed_calls_raw_stage;
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SPEAKER_MAPPING (
	RELATIVE_PATH VARCHAR(16777216),
	SPEAKER VARCHAR(16777216),
	SPEAKER_NAME VARCHAR(16777216)
);

CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.speaker_mapping_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/speaker_mapping.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.speaker_mapping_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SPEAKER_MAPPING;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SPEAKER_MAPPING
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.speaker_mapping_stage/speaker_mapping.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.speaker_mapping_stage;
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.AI_TRANSCRIBE_NO_TIME (
	RELATIVE_PATH VARCHAR(16777216),
	TEXT VARCHAR(16777216),
	DURATION FLOAT
);

CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ai_transcribe_no_time_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/ai_transcribe_no_time.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ai_transcribe_no_time_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.AI_TRANSCRIBE_NO_TIME;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.AI_TRANSCRIBE_NO_TIME
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ai_transcribe_no_time_stage/ai_transcribe_no_time.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ai_transcribe_no_time_stage;

-- 2. Load TRANSCRIBED_EARNINGS_CALLS_WITH_SENTIMENT
-- =====================================================
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.transcribed_calls_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/transcribed_earnings_calls_with_sentiment.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.transcribed_calls_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.TRANSCRIBED_EARNINGS_CALLS_WITH_SENTIMENT;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.TRANSCRIBED_EARNINGS_CALLS_WITH_SENTIMENT
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.transcribed_calls_stage/transcribed_earnings_calls_with_sentiment.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.transcribed_calls_stage;

-- 3. Load TRANSCRIPTS_BY_MINUTE
-- =====================================================
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.transcripts_minute_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/transcripts_by_minute.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.transcripts_minute_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.TRANSCRIPTS_BY_MINUTE;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.TRANSCRIPTS_BY_MINUTE
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.transcripts_minute_stage/transcripts_by_minute.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.transcripts_minute_stage;

-- 4. Load SENTIMENT_ANALYSIS
-- =====================================================
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.sentiment_analysis_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/sentiment_analysis.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.sentiment_analysis_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SENTIMENT_ANALYSIS;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SENTIMENT_ANALYSIS
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.sentiment_analysis_stage/sentiment_analysis.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.sentiment_analysis_stage;

-- 5. Load INFOGRAPHICS_FOR_SEARCH
-- =====================================================
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.infographics_search_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/infographics_for_search.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.infographics_search_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.INFOGRAPHICS_FOR_SEARCH;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.INFOGRAPHICS_FOR_SEARCH
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.infographics_search_stage/infographics_for_search.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.infographics_search_stage;

-- 6. Load ANALYST_REPORTS
-- =====================================================
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.analyst_reports_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/analyst_reports.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.analyst_reports_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ANALYST_REPORTS;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ANALYST_REPORTS
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.analyst_reports_stage/analyst_reports.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.analyst_reports_stage;

-- 7. Load INFOGRAPHIC_METRICS_EXTRACTED (DOCUMENT_AI schema)
-- =====================================================
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.infographic_metrics_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/infographic_metrics_extracted.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.infographic_metrics_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.INFOGRAPHIC_METRICS_EXTRACTED;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.INFOGRAPHIC_METRICS_EXTRACTED
FROM @{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.infographic_metrics_stage/infographic_metrics_extracted.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.infographic_metrics_stage;

-- 8. Load ANALYST_REPORTS_ALL_DATA (DOCUMENT_AI schema)
-- =====================================================
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.analyst_reports_all_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/analyst_reports_all_data.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.analyst_reports_all_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.ANALYST_REPORTS_ALL_DATA;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.ANALYST_REPORTS_ALL_DATA
FROM @{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.analyst_reports_all_stage/analyst_reports_all_data.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.analyst_reports_all_stage;
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.PARSED_ANALYST_REPORTS (
	RELATIVE_PATH VARCHAR(16777216),
	EXTRACTED_DATA VARIANT
);

CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.parsed_analyst_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/parsed_analyst_reports.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.parsed_analyst_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.PARSED_ANALYST_REPORTS;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.PARSED_ANALYST_REPORTS
FROM @{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.parsed_analyst_stage/parsed_analyst_reports.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.parsed_analyst_stage;
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.AI_EXTRACT_ANALYST_REPORTS_ADVANCED (
	RELATIVE_PATH VARCHAR(16777216),
...
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.ai_extract_advanced_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/ai_extract_analyst_reports_advanced.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.ai_extract_advanced_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.AI_EXTRACT_ANALYST_REPORTS_ADVANCED;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.AI_EXTRACT_ANALYST_REPORTS_ADVANCED
FROM @{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.ai_extract_advanced_stage/ai_extract_analyst_reports_advanced.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.ai_extract_advanced_stage;
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.REPORT_PROVIDER_SUMMARY (
	NAME_OF_REPORT_PROVIDER VARCHAR(16777216),
	SUMMARY VARCHAR(16777216)
);

CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.report_summary_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/report_provider_summary.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.report_summary_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.REPORT_PROVIDER_SUMMARY;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.REPORT_PROVIDER_SUMMARY
FROM @{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.report_summary_stage/report_provider_summary.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.report_summary_stage;

-- 9. Load FINANCIAL_REPORTS (DOCUMENT_AI schema)
-- =====================================================
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.financial_reports_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/financial_reports.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.financial_reports_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.FINANCIAL_REPORTS;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.FINANCIAL_REPORTS
FROM @{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.financial_reports_stage/financial_reports.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.financial_reports_stage;

-- 10. Load unique_transcripts
-- =====================================================
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.unique_transcripts_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/unique_transcripts.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.unique_transcripts_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.unique_transcripts;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.unique_transcripts
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.unique_transcripts_stage/unique_transcripts.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.unique_transcripts_stage;

-- =====================================================
-- Verification Queries
-- =====================================================
SELECT 'AI_TRANSCRIPTS_ANALYSTS_SENTIMENTS' AS TABLE_NAME, COUNT(*) AS ROWS_LOADED FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.AI_TRANSCRIPTS_ANALYSTS_SENTIMENTS
UNION ALL
SELECT 'TRANSCRIBED_EARNINGS_CALLS', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.TRANSCRIBED_EARNINGS_CALLS
UNION ALL
SELECT 'SPEAKER_MAPPING', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SPEAKER_MAPPING
UNION ALL
SELECT 'AI_TRANSCRIBE_NO_TIME', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.AI_TRANSCRIBE_NO_TIME
UNION ALL
SELECT 'TRANSCRIBED_EARNINGS_CALLS_WITH_SENTIMENT', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.TRANSCRIBED_EARNINGS_CALLS_WITH_SENTIMENT
UNION ALL
SELECT 'TRANSCRIPTS_BY_MINUTE', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.TRANSCRIPTS_BY_MINUTE
UNION ALL
SELECT 'SENTIMENT_ANALYSIS', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SENTIMENT_ANALYSIS
UNION ALL
SELECT 'INFOGRAPHICS_FOR_SEARCH', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.INFOGRAPHICS_FOR_SEARCH
UNION ALL
SELECT 'ANALYST_REPORTS', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ANALYST_REPORTS
UNION ALL
SELECT 'INFOGRAPHIC_METRICS_EXTRACTED', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.INFOGRAPHIC_METRICS_EXTRACTED
UNION ALL
SELECT 'ANALYST_REPORTS_ALL_DATA', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.ANALYST_REPORTS_ALL_DATA
UNION ALL
SELECT 'FINANCIAL_REPORTS', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.FINANCIAL_REPORTS
UNION ALL
SELECT 'PARSED_ANALYST_REPORTS', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.PARSED_ANALYST_REPORTS
UNION ALL
SELECT 'STOCK_PRICES', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.STOCK_PRICES
UNION ALL
SELECT 'AI_EXTRACT_ANALYST_REPORTS_ADVANCED', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.AI_EXTRACT_ANALYST_REPORTS_ADVANCED
UNION ALL
SELECT 'REPORT_PROVIDER_SUMMARY', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.REPORT_PROVIDER_SUMMARY
UNION ALL
SELECT 'unique_transcripts', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.unique_transcripts;
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.call_embeds (
	RELATIVE_PATH VARCHAR(16777216),
	SUMMARY VARCHAR(16777216),
	TEXT VARCHAR(16777216),
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.EMAIL_PREVIEWS_EXTRACTED (
...
-- 11. Load call_embeds
-- =====================================================
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.call_embeds_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/call_embeds.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.call_embeds_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.call_embeds;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.call_embeds
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.call_embeds_stage/call_embeds.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.call_embeds_stage;

-- 12. Load EMAIL_PREVIEWS_EXTRACTED
-- =====================================================
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.email_extracted_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/email_previews_extracted.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.email_extracted_stage 
    auto_compress = false 
    overwrite = true;

TRUNCATE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.EMAIL_PREVIEWS_EXTRACTED;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.EMAIL_PREVIEWS_EXTRACTED
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.email_extracted_stage/email_previews_extracted.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.email_extracted_stage;
...
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.EMAIL_PREVIEWS_EXTRACTED TO ROLE {{ env.EVENT_ATTENDEE_ROLE }};
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.EMAIL_PREVIEWS_EXTRACTED TO ROLE PUBLIC;

-- Analyst reports search service table
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ANALYST_REPORTS TO ROLE {{ env.EVENT_ATTENDEE_ROLE }};
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ANALYST_REPORTS TO ROLE PUBLIC;

-- Infographics search service table
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.INFOGRAPHICS_FOR_SEARCH TO ROLE {{ env.EVENT_ATTENDEE_ROLE }};
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.INFOGRAPHICS_FOR_SEARCH TO ROLE PUBLIC;



-- Sentiment analysis search service (uses view, grant on underlying tables)
GRANT SELECT ON VIEW {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SENTIMENT_WITH_TRANSCRIPTS_FOR_SEARCH TO ROLE {{ env.EVENT_ATTENDEE_ROLE }};
GRANT SELECT ON VIEW {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SENTIMENT_WITH_TRANSCRIPTS_FOR_SEARCH TO ROLE PUBLIC;
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.unique_transcripts TO ROLE {{ env.EVENT_ATTENDEE_ROLE }};
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.unique_transcripts TO ROLE PUBLIC;
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ai_transcripts_analysts_sentiments TO ROLE {{ env.EVENT_ATTENDEE_ROLE }};
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ai_transcripts_analysts_sentiments TO ROLE PUBLIC;
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.FULL_TRANSCRIPTS AS
SELECT 
    RELATIVE_PATH,
    SUMMARY,
    TEXT,
...
FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.call_embeds;

-- Full transcripts search service table
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.FULL_TRANSCRIPTS TO ROLE {{ env.EVENT_ATTENDEE_ROLE }};
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.FULL_TRANSCRIPTS TO ROLE PUBLIC;

-- Update ANALYST_REPORTS with URL placeholder
-- (Table already created and loaded above, just adding URL column)
ALTER TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ANALYST_REPORTS 
ADD COLUMN IF NOT EXISTS URL VARCHAR(16777216);

UPDATE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ANALYST_REPORTS 
...
ALTER TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.INFOGRAPHICS_FOR_SEARCH 
ADD COLUMN IF NOT EXISTS URL VARCHAR(16777216);

UPDATE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.INFOGRAPHICS_FOR_SEARCH 
SET URL = 'PRESIGNED_URL_PLACEHOLDER';
...
CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SOCIAL_MEDIA_NRNT (
    TIMESTAMP TIMESTAMP,
    PLATFORM VARCHAR,
...
CREATE TEMPORARY STAGE IF NOT EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.social_media_stage
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
PUT file:///{{ env.CI_PROJECT_DIR }}/dataops/event/DATA/social_media_nrnt_collapse.csv 
    @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.social_media_stage 
    auto_compress = false 
    overwrite = true;

COPY INTO {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SOCIAL_MEDIA_NRNT
FROM @{{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.social_media_stage/social_media_nrnt_collapse.csv
FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
...
DROP STAGE IF EXISTS {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.social_media_stage;
...
FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SOCIAL_MEDIA_NRNT;

-- Grant permissions
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SOCIAL_MEDIA_NRNT TO ROLE {{ env.EVENT_ATTENDEE_ROLE }};
GRANT SELECT ON TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.SOCIAL_MEDIA_NRNT TO ROLE PUBLIC;
...
CREATE OR REPLACE TASK {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.refresh_full_transcripts_task
    WAREHOUSE = {{ env.EVENT_WAREHOUSE }}
...
AS
    ...
    SELECT * EXCLUDE EMBED, 
           GET_PRESIGNED_URL(@{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.EARNINGS_CALLS, RELATIVE_PATH, 604800) AS URL  
    FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.call_embeds;

-- Task to refresh ANALYST_REPORTS URLs
CREATE OR REPLACE TASK {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.refresh_analyst_reports_url_task
    WAREHOUSE = {{ env.EVENT_WAREHOUSE }}
    SCHEDULE = '7200 MINUTE'  -- Run every 5 days
    COMMENT = 'Refreshes url for analyst reports table every 5 days to regenerate presigned URLs which expire after 7 days'
AS
    CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.ANALYST_REPORTS AS
    SELECT *, 
           GET_PRESIGNED_URL(@{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.ANALYST_REPORTS, RELATIVE_PATH, 604800) AS URL  
    FROM {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.ANALYST_REPORTS_ALL_DATA;

-- Task to refresh INFOGRAPHICS_FOR_SEARCH URLs
CREATE OR REPLACE TASK {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.refresh_infographics
    WAREHOUSE = {{ env.EVENT_WAREHOUSE }}
    SCHEDULE = '7200 MINUTE'  -- Run every 5 days
    COMMENT = 'Refreshes url for infographics table every 5 days to regenerate presigned URLs which expire after 7 days'
AS
    CREATE OR REPLACE TABLE {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.INFOGRAPHICS_FOR_SEARCH AS 
    SELECT *, 
           GET_PRESIGNED_URL(@{{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.INFOGRAPHICS, RELATIVE_PATH, 604800) AS URL 
    FROM {{ env.EVENT_DATABASE }}.{{ env.DOCUMENT_AI_SCHEMA }}.VW_INFOGRAPHIC_METRICS;

-- Resume all tasks
ALTER TASK {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.refresh_full_transcripts_task RESUME;
ALTER TASK {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.refresh_analyst_reports_url_task RESUME;
ALTER TASK {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.refresh_infographics RESUME;
*/

-- =====================================================
-- Final Verification Queries for Search Service Tables
-- =====================================================
SELECT 'call_embeds' AS TABLE_NAME, COUNT(*) AS ROWS_LOADED FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.call_embeds
UNION ALL
SELECT 'EMAIL_PREVIEWS_EXTRACTED', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.EMAIL_PREVIEWS_EXTRACTED
UNION ALL
SELECT 'FULL_TRANSCRIPTS', COUNT(*) FROM {{ env.EVENT_DATABASE }}.{{ env.EVENT_SCHEMA }}.FULL_TRANSCRIPTS;
